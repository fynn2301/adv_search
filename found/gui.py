...
H_SPACING = 8
V_SPACING = 1
WINDOW_WIDTH = (H_MARGIN_DISTANCE * 2) + \
    (CIRCLE_DIAMETER * 13) + (H_SPACING * 12)
WINDOW_HEIGHT = (V_MARGIN_DISTANCE * 2) + \
...
def draw_board(board, display_surface):
    ...
    for row in range(0, 17):
        ...
        x_coord_short = int(H_MARGIN_DISTANCE +
        ...
        for circle_in_a_row in range(0, 13):
            if row % 2 == 0:
                ...
                if [row, circle_in_a_row * 2] in destinations:
                    color_destination(
                        display_surface, x_coord_long, y_coord, row, circle_in_a_row, destinations)
                else:
...
def color_circle(board_value, display_surface, x_coord, y_coord):
    ...
    if board_value == -1:
        pg.draw.circle(display_surface, BACKGROUND,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 0:
        pg.draw.circle(display_surface, EMPTY_CELL,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 1:
        pg.draw.circle(display_surface, PLAYER1_BLACK,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 2:
        pg.draw.circle(display_surface, PLAYER2_WHITE,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 3:
        pg.draw.circle(display_surface, PLAYER3_RED,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 4:
        pg.draw.circle(display_surface, PLAYER4_BLUE,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 5:
        pg.draw.circle(display_surface, PLAYER5_GREEN,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
    if board_value == 6:
        pg.draw.circle(display_surface, PLAYER6_YELLOW,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)
...
def color_destination(display_surface, x_coord_long, y_coord, row, circle_in_a_row, destinations):
    ...
    if [row, circle_in_a_row * 2] == destinations[0]:
        pg.draw.circle(display_surface, PLAYER3_RED,
                       (x_coord_long, y_coord), CIRCLE_RADIUS, 0)
...
def find_circle_from(x, y):
    ...
    if x % 2 == 0:
        circle_x = int(H_MARGIN_DISTANCE + CIRCLE_RADIUS +
                       (CIRCLE_DIAMETER + H_SPACING) * (y / 2))
    else:
        circle_x = int(H_MARGIN_DISTANCE + CIRCLE_DIAMETER + (H_SPACING / 2) + (CIRCLE_DIAMETER + H_SPACING) * ((y - 1)
                                                                                                                / 2))
    circle_y = V_MARGIN_DISTANCE + CIRCLE_RADIUS + \
        (CIRCLE_DIAMETER + V_SPACING) * x
